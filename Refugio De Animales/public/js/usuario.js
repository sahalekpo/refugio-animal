...
async function consultarSolicitud(cedula) {
  ...
  try {
    ...
    box.innerHTML = data.postulaciones.map(p => {
      ...
      const certBtns = p.certificado_codigo ? `
        <div class="cert-actions">
          <a class="btn btn-primary" href="${p.certificado_url}" target="_blank">Ver mi certificado</a>
          <a class="btn btn-outline" href="/${p.certificado_pdf}" target="_blank">Descargar PDF</a>
        </div>
        <p class="cert-hint">Código: <strong>${p.certificado_codigo}</strong></p>
      ` : (p.estado_postulacion === 'Pendiente'
        ? '<p class="hint">Su encuesta está siendo evaluada por el refugio.</p>'
        : '');
