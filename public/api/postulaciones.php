...
try {
    ...
    if ($method === 'POST' && $action === 'postular') {
        ...
        $stmt = $pdo->prepare('
            SELECT a.id_animal, a.nombre, es.nombre_estado, e.nombre_especie
            FROM animales a
            INNER JOIN estados es ON a.id_estado = es.id_estado
            INNER JOIN especies e ON a.id_especie = e.id_especie
            WHERE a.id_animal = ?
    ...
    if ($method === 'POST' && $action === 'aprobar') {
        ...
        ");
        $stmtOtras->execute([$post['id_animal'], $data['id_postulacion']]);
        $otrasPendientes = $stmtOtras->fetchAll();
        ...
        foreach ($otrasPendientes as $otra) {
            $notificacionesRechazo[] = notificarPostulacionRechazada($otra, $post['animal'], 'otra_aprobada');
        }
        ...
        jsonResponse([
            ...
            'rechazados_count' => count($otrasPendientes)
        ], 201);
    }
    ...
    if ($method === 'POST' && $action === 'rechazar') {
        ...
        $email = notificarPostulacionRechazada($post, $post['animal'], 'manual');
